    ...
    if (request.POST):
        ...
        for i in ids:
            if i.user_id.user_id.username in present:
                a = Attendance(date=datetime.datetime.now().date(), present = True,user_id_id = i.user_id.user_id.id, on_duty = False)        
            else:
	        if i.user_id.user_id.username in on_duty:
                    a = Attendance(date=datetime.datetime.now().date(), present = False,user_id_id = i.user_id.user_id.id,on_duty = True)                      
                else:
		    a = Attendance(date=datetime.datetime.now().date(), present = False,user_id_id = i.user_id.user_id.id,on_duty = False)                      
            a.save()
    return HttpResponseRedirect("/")
