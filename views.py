...
def edirectory_courses(request):
	...
	t = User.objects.get(user_id = u.id)
	p = Profile.objects.get(user_id = u.id)
	return render_to_response('edirectory_course.html')
    except:
	return render_to_response('error.html')
...
def feedback_details(request):
    if (request.POST):
        ...
        t = User.objects.get(user_id = u.id)
        p = Profile.objects.get(user_id = u.id)
        every  = Offered.objects.filter(user_id = u.id)
        ...
        tot=[]
        for e in every:
	    c = Feedback.objects.filter(every_id = e).filter(start_date__gte = start_date).filter(end_date__lte = end_date).aggregate(Avg('avg_content_rat'))
	    p = Feedback.objects.filter(every_id = e).filter(start_date__gte = start_date).filter(end_date__lte = end_date).aggregate(Avg('avg_present_rat'))
